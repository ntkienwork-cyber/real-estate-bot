...
# Populated by load_market_data() below
MARKET_DATA: dict = {}
MACRO_DATA: dict = {}
MARKET_DATA_UPDATED: _dt.date = _dt.date.today()
CREDIT_GROWTH_YOY: float = 0.0
MORTGAGE_RATE_CURRENT: float = 0.0
MORTGAGE_RATE_TREND: str = "stable"
_META: dict = {}


def load_market_data() -> None:
    global MARKET_DATA, MACRO_DATA, MARKET_DATA_UPDATED
    global CREDIT_GROWTH_YOY, MORTGAGE_RATE_CURRENT, MORTGAGE_RATE_TREND, _META

    _path = os.path.join(os.path.dirname(__file__), "market_data.json")
    with open(_path, encoding="utf-8") as _f:
        _raw = json.load(_f)

    _META = _raw["meta"]
    MARKET_DATA_UPDATED = _dt.date.fromisoformat(_META["updated"])
    MARKET_DATA = _raw["market"]
    MACRO_DATA = _raw["macro"]
    _c = _raw["constants"]
    CREDIT_GROWTH_YOY = _c["CREDIT_GROWTH_YOY"]
    MORTGAGE_RATE_CURRENT = _c["MORTGAGE_RATE_CURRENT"]
    MORTGAGE_RATE_TREND = _c["MORTGAGE_RATE_TREND"]
...
def get_data_freshness() -> dict:
    ...
    age   = (today - MARKET_DATA_UPDATED).days
    return {
        "market_updated":  MARKET_DATA_UPDATED.isoformat(),
        "macro_updated":   MARKET_DATA_UPDATED.isoformat(),
        "market_stale":    age > DATA_STALE_DAYS,
        "macro_stale":     age > DATA_STALE_DAYS,
        "market_age_days": age,
        "macro_age_days":  age,
        "updated_by":      _META.get("updated_by", "manual"),
